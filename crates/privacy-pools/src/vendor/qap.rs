//! The snarkjs-compatible QAP witness map (`CircomReduction`).
//!
//! Vendored verbatim from `ark-circom` (arkworks-rs/circom-compat) v0.5.0,
//! `src/circom/qap.rs` (MIT OR Apache-2.0). Depends only on arkworks — no
//! `wasmer`. See [`super::zkey`] for why we vendor instead of depending on
//! `ark-circom`.
