//! snarkjs `.zkey` → arkworks `ProvingKey<Bn254>` reader.
//!
//! Vendored verbatim from `ark-circom` (arkworks-rs/circom-compat) v0.5.0,
//! `src/zkey.rs`, which is dual-licensed MIT OR Apache-2.0. We vendor it (rather
//! than depend on `ark-circom`) because `ark-circom` pulls in `wasmer` /
//! `wasmer-wasix` as mandatory dependencies for its WASM witness calculator —
//! the exact thing circom-witnesscalc lets us avoid. This file, by contrast,
//! depends only on arkworks + `byteorder` + `num-traits`.
//!
//! Modifications from upstream:
//!   * removed the `#[cfg(test)]` module (its only `wasmer` usage),
//!   * `G1Affine::new` / `G2Affine::new` → `*::new_unchecked` (the bundled zkey
//!     is a pinned, trusted artifact, so we skip the per-point on-curve /
//!     subgroup checks — a large speedup when loading the 17 MB withdraw key).
//!
//! Each ZKey file is broken into sections:
//!  Header(1) · HeaderGroth(2) · IC(3) · Coeffs(4) · PointsA(5) · PointsB1(6)
//!  PointsB2(7) · PointsC(8) · PointsH(9) · Contributions(10)
...
fn deserialize_g1<R: Read>(reader: &mut R) -> IoResult<G1Affine> {
    ...
    if infinity {
        ...
        // trusted, pinned artifact: skip on-curve/subgroup validation
